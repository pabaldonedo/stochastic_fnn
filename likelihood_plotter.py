...
def main():
    ...
    assert type(n) is IntType, "Unrecognized type for n value. It must be an integer. Provided: {0!r}".format(n)
    ...
    if test_fname is not None:
        assert type(ntest) is IntType, "Unrecognized type for ntest value. It must be an integer. Provided: {0!r}".format(ntest)
    ...
    ax.plot(train_log_likelihood[:,0], train_log_likelihood[:,1], color=colors[0])

    #For showing mean log likelihood values in the right y-axis
    ax2 = ax.twinx()
    ax2.plot(train_log_likelihood[:,0], train_log_likelihood[:,1]*1./n, color=colors[0], alpha=0)
    ...
    if test_fname is not None:
        ...
        ax.plot(test_log_likelihood[:,0], test_log_likelihood[:,1], color=colors[1])
        ax2.plot(test_log_likelihood[:,0], test_log_likelihood[:,1]*1./ntest, color=colors[1], alpha=0)
