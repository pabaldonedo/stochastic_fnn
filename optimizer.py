...
class Optimizer():
    ...
    def fiting_variables(self, batch_size, train_set_x, test_set_x=None, sample_axis=0):
        ...
        :type test_set_x: theano.sharedvariale or None.
        :param test_set_x: test datasets used for computing testing error while training.

        :type sample_axis: integer.
        :param sample_axis: which axis in train_set_x and test_set_x corresponds to
                                                                "training samples".
        """
...
class GradientBased(Optimizer):
    ...
    def fit(self, x, y, x_train, y_train, batch_size, cost, theta, n_epochs,
        ...
        :type x: numpy.array of dimensionality:
                [sequence length, #training samples, input dimensionality] or
                [#training samples, input dimensionality].
        :param x: input variable of the rnn.
        ...
        :type theta: list
        :param theta: List containing the parameters.

        :type n_epcohs: integers
        :param n_epochs: number of epochs to train.

        :type compute_error: theano function.
        :param compute_error: computes error given predicted output and true output.

        :type call_back: python function.
        :param call_back: call back function to call for printing information.

        :type x_test: numpy.array of dimensionality:
                [sequence length, #training samples, input dimensionality] or
                [#training samples, input dimensionality].
        :param x_test: test input set.

        :type y_test: numpy.array.
        :param y_test: test output goal.

        :type validate_every: integer.
        :param validate_every: tells every how many epochs the fit functions reports to the
                               call_back function.

        :type extra_train_givens: dictionary.
        :param extra_train_givens: in the training function extra necessary
                                    values and theano variables.

        :type chunk_size: integer or None.
        :param chunk_size: size of memory chunks load at a time in memory. If None,
        ...
        if chunk_size is None:
            ...
            n_chunks = int(
                np.ceil(x_train.shape[sample_axis] * 1. / chunk_size))
        ...
        if x_test is not None and y_test is not None:
            ...
            test_n_chunks = int(
                np.ceil(x_test.shape[sample_axis] * 1. / chunk_size))
        else:
            test_set_x = None
            test_set_y = None
        ...
        while epoch < n_epochs:
            ...
            for chunk in xrange(n_chunks):
                ...
                train_set_y.set_value(y_train.take(xrange(chunk * chunk_size, min(
                    (chunk + 1) * chunk_size, y_train.shape[sample_axis])), axis=sample_axis))
            ...
            if self.test_availavility:
                ...
                for chunk in xrange(test_n_chunks):
                    test_set_x.set_value(x_test.take(xrange(
                        chunk * chunk_size, min((chunk + 1) * chunk_size, x_test.shape[sample_axis])), axis=sample_axis))
                    test_set_y.set_value(y_test.take(xrange(
                        chunk * chunk_size, min((chunk + 1) * chunk_size, y_test.shape[sample_axis])), axis=sample_axis))
                    ...
                    for minibatch_idx in xrange(n_test_batches):
                        if sample_axis == 0:
                            ...
                            batch_log_likelihood = compute_error(test_set_x[:, minibatch_idx * batch_size:min((minibatch_idx + 1) * batch_size, this_chunk_size)].eval(),
                                                                 test_set_y[:, minibatch_idx * batch_size:min((minibatch_idx + 1) * batch_size, this_chunk_size)].eval())
                        this_batch_size = min(
                            (minibatch_idx + 1) * batch_size, this_chunk_size) - minibatch_idx * batch_size
...
class RMSProp(GradientBased):
    ...
    def __init__(self, learning_rate, rho, epsilon):
        ...
        assert type(self.learning_rate) is FloatType or IntType, "Learning rate must be an integer or float: {0!r}".format(
            self.learning_rate)
        assert 0 < self.learning_rate, "Learning rate must be positive: {0!r}".format(
            self.learning_rate)
        self.rho = rho
        assert type(
            self.rho) is FloatType or IntType, "Rho decay must be an integer or float: {0!r}".format(self.rho)
        assert 0 < self.rho, "Rho decay must be positive: {0!r}".format(
            self.rho)
        self.epsilon = epsilon
        assert type(self.epsilon) is FloatType or IntType, "Epsilon must be an integer or float: {0!r}".format(
            self.epsilon)
        assert 0 < self.epsilon, "Epsilon must be positive: {0!r}".format(
            self.epsilon)
        self.opt_parameters = {
            'opt': 'RMSProp', 'lr': self.learning_rate, 'rho': self.rho, 'e': self.epsilon}
    ...
    def get_updates(self, theta, cost=None, gtheta=None):
        ...
        theta_upd = theta - self.learning_rate * \
            gtheta / T.sqrt(rms_upd + self.epsilon)
...
class AdaGrad(GradientBased):
    ...
    def __init__(self, learning_rate, epsilon):
        ...
        assert 0 < self.epsilon, "Epsilon must be positive: {0!r}".format(
            self.epsilon)
        self.opt_parameters = {
            'opt': 'AdaGrad', 'learning_rate': self.learning_rate, 'epsilon': self.epsilon}
    ...
    def get_updates(self, theta, cost=None, gtheta=None):
        ...
        theta_upd = theta - self.learning_rate * \
            gtheta / T.sqrt(g2_sum_upd + self.epsilon)
...
class Adam(GradientBased):
    ...
    def __init__(self, step_size, b1, b2, e):
        ...
        assert type(self.step_size) is FloatType or IntType, "Step size must be an integer or float: {0!r}".format(
            self.step_size)
        assert 0 < self.step_size, "Step size must be positive: {0!r}".format(
            self.step_size)
        self.b1 = b1
        assert type(
            self.b1) is FloatType or IntType, "B1 must be a float or integer: {0!r}".format(self.b1)
        assert 0 <= self.b1 < 1, "B1 must be in range [0, 1): {0!r}".format(
            self.b1)
        self.b2 = b2
        assert type(
            self.b2) is FloatType or IntType, "B2 must be a float or integer: {0!r}".format(self.b2)
        assert 0 <= self.b2 < 1, "B2 must be in range [0, 1): {0!r}".format(
            self.b2)
        self.e = e
        assert type(
            self.e) is FloatType or IntType, "Epsilon must be an integer or float: {0!r}".format(self.e)
...
class SGD(GradientBased):
    ...
    def __init__(self, lr_decay_schedule, lr_decay_parameters, momentum_type, momentum=None):
        ...
        """
        self.lr_update = self.learning_rate_scheduler(
            lr_decay_schedule, lr_decay_parameters)
        self.momentum_type = momentum_type
        self.momentum_types = ['classic', 'nesterov', 'none']
        assert self.momentum_type in momentum_type, "Momentum type not implemented: {0!r}. Try wiht one of the following: {1}".format(
            self.momentum_type, self.momentum_types)

        assert type(
            momentum) is FloatType or IntType, "Momentum parameter must be an int or float: {0!r}".format(momentum)
        if self.momentum_type != self.momentum_types[2]:
            assert momentum >= 0, "Momentum must be >=0: {0!r}".format(
                momentum)
        self.momentum_update = lambda epoch: momentum  # TODO momentum schedules
        logging.info('SGD optimizer object created')
        self.opt_parameters = {'opt': 'SGD', 'mom': momentum, 'momentum_type': self.momentum_type,
                               'lr_decay_schedule': lr_decay_schedule, 'lr_decay_parameters': lr_decay_parameters}
        ...
        logging.info('Optimizer loaded. Type: {0}, momentum_type: {1},'
                     ...
                     ' learning rate decay parameters([n(0), r, c]: {4})'.format(self.opt_parameters['opt'],
                                                                                 self.opt_parameters[
                                                                                     'momentum_type'],
                                                                                 self.opt_parameters['mom'], self.opt_parameters[
                                                                                     'lr_decay_schedule'],
    ...
    def learning_rate_scheduler(self, lr_decay_schedule, lr_decay_parameters):
        ...
        self.lr_decay_schedules = ['constant',
                                   'multiplication', 'exponential', 'power']
        ...
        assert type(init_lr) is FloatType or IntType,\
            "Initial learning_rate value must be float or int: {0!r}".format(
                init_lr)

        assert init_lr >= 0, "Initial learning_rate must be >=0: {0!r}".format(
            init_lr)
        ...
        assert type(decay_rate) is FloatType or IntType,\
            "Decay rate for learning rate value must be float or int: {0!r}".format(
                decay_rate)

        assert decay_rate >= 0, "Decay rate for learning rate must be >=0: {0!r}".format(
            decay_rate)
        ...
        if lr_decay_schedule == self.lr_decay_schedules[3]:
            ...
            assert type(c) is FloatType or IntType, \
                "c parameter in Power Schedule for lr must be float or int: {0!r}".format(
                    type(c))

            assert c >= 0, "c in Power Schedule for lr must be >=0: {0!r}".format(
                c)
    ...
    def get_updates(self, theta, cost=None, gtheta=None):
        ...
        if self.momentum_type != self.momentum_types[2]:
            v = theano.shared(value=np.zeros(
                theta.shape.eval(), dtype=theano.config.floatX))
